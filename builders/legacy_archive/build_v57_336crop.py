"""v57: v56 ctft overlap-max + 336 query crop-max against the 336 iNat bank.

336 query views: outputs/emb_{test,unseen}_fullft336_cropviews.pt
Optional crop-bank: outputs/inat_photo_bank_fullft336_cropviews.pt (used if present).
Single pipeline, f=0.60, tau=1.8, argmax over full 17393 classes.

  conda activate onet && python builders/build_v57_336crop.py
"""
import json
import os
import pickle
...
BANK_336_CROP = 'outputs/inat_photo_bank_fullft336_cropviews.pt'
...
CROP336_TEST = 'outputs/emb_test_fullft336_cropviews.pt'
CROP336_UNS = 'outputs/emb_unseen_fullft336_cropviews.pt'
...
def main():
    print('=== Building v57 336 query crop-max + ctft overlap-max ===', flush=True)
    for p in (CROP_TEST, CROP_UNS, OSTRIP_TEST, OSTRIP_UNS, CROP336_TEST, CROP336_UNS, BANK_CTFT, BANK_336):
    ...
    packs336 = [
        load_cropviews(CROP336_TEST, VIEW_KEYS),
        load_cropviews(CROP336_UNS, VIEW_KEYS),
    ]
    fb336 = Q['fullft336shift'].cpu()
    view336 = []
    n_fb336 = 0
    for k in VIEW_KEYS:
        rows = []
        for i, fn in enumerate(all_files):
            ...
            if hit is None:
                ...
                hit = fb336[i]
            rows.append(hit)
        view336.append(F.normalize(torch.stack(rows).float(), dim=-1).to(dev))
    print(f'336 crop views aligned fallback_slots={n_fb336}', flush=True)
    bank336_path = BANK_336_CROP if os.path.exists(BANK_336_CROP) else BANK_336
    print(f'scoring 336 crop-max bank {bank336_path} ...', flush=True)
    Sraw_336 = score_bank_maxviews(view336, torch.load(bank336_path, weights_only=False)['bank'], other_list)
