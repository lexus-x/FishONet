...
current best-in-repo real-shipped seen re-ranker's holdout number).
  ...
  conda activate onet && python research/seen_linear_probe_v101.py
"""
...
def build_member(t, kept, s2i, trby, train, Tseen, S):
    ...
    seen_inat_v95.py / seen_reweight_v98.py's main(), factored out so the probe leg (t=PROBE_LEG)
    can also hand back its raw per-image (features, labels, prototype, count) for training."""
    idx, feats, _ = train[t]
...
def main():
    ...
    yval = torch.tensor([s2i[c] for c in sp['cls'][:sp['n_seen']]], device=dev)
