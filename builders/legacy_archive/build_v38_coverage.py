"""v38: v36 + iNaturalist research-grade image-prototype leg on the UNSEEN route.

Rationale (HANDOFF §8 "highest-EV lever"): the v36 unseen recognizer is text-limited
(BioCLIP cannot visually resolve novel species from taxon text). Real reference images
of each species give an image->image channel that is far more discriminative.

External data: iNaturalist observations, quality_grade=research, fetched by scientific
name (COMPETITION_RULES §3.2 external data allowed with disclosure; iNat is a general
biodiversity platform, NOT a fish-specific model/dataset). Photos used only as per-class
visual prototypes — never matched to eval images, never used for split membership.
Near-duplicate probe (research/inat_dup_probe.py): 0% of train queries match any iNat
image at cosine>0.95 (max 0.945) -> genuinely different photos, no leakage.

Proxy (pseudo-unseen hard-sim, research/inat_proto_proxy.py, ctftshift protos, distractors
also covered — the trustworthy proxy; the old gold-only +31.75 was inflated):
  v36 text stack (minus TB)            31.45
  + iNat img leg (w=1.5 / 2 / 3)       41.11 / 42.23 / 43.53   (+9.66 / +10.79 / +12.08)
  class-disjoint CV (covered-gold)     37.84 -> 52.52 (+14.68)  <- no overfit
REAL (2026-07-24): v36 49.04 -> w1.5 50.12 -> w2 50.32 (seen flat 78.95; unseen 10.42->13.35).
0% near-duplicate leakage. Single pipeline: img leg added only to the unseen-route
scoring; gate + seen route unchanged from v36. Disclose iNaturalist.
"""
import json
import os
import pickle
...
TAXABIND_W = 1.0
IMG_W = float(os.environ.get('IMG_W', '2.0'))
PROTO_PATH = os.environ.get('PROTO_PATH', 'outputs/protos_ctftshift_merged.pt')
...
# ---- iNaturalist image-prototype leg (UNSEEN route only) ----
...
Po = protoC[other_idx]
haso = hasp[other_idx]
Simg = Q['ctftshift'] @ Po.t()
Simg = Simg.masked_fill(~haso.unsqueeze(0), -1e4)
img_leg = dbnorm(Simg)
text_unseen_only = text_unseen_only + IMG_W * img_leg
print(f'iNat protos: {int(hasp.sum())}/{NCLS} classes covered; '
      f'unseen-route coverage {int(haso.sum())}/{len(other_idx)} '
      f'({100*haso.float().mean():.1f}%); IMG_W={IMG_W}')
...
tag = f'v38_coverage_w{IMG_W:g}_sink72'
