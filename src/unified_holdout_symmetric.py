"""Symmetric compliant single-pipeline holdout: seen-block uses ONLY backbones
cached for ALL 35665 real eval images (baseline H + L). No ft/cap (those are
only cached for the seen-eval/test folder, which would reintroduce a
backbone-availability routing asymmetry -- see v21 finding). This is the
formula the real submission will use, uniformly, for every image.

Same split/gate/frontier structure as unified_holdout.py.
"""
...
seen_block = zc(sc_h) + 0.5 * zc(sc_l)   # [n_q, S] symmetric: H (+taxon) + 0.5*L
...
seen_only_acc = oracle_seen_acc  # identical by construction (seen_block already kept_seen-only)
...
print(f'SANITY(symmetric): oracle_seen={oracle_seen_acc:.2f} | oracle_unseen={oracle_unseen_acc:.2f} '
...
frontier = []
gammas = [0, 1, 2, 3, 5, 7, 10, 12, 15, 20, 25, 30, 40, 50]
w_texts = [0.25, 0.5, 1.0]
for w_text in w_texts:
    for gamma in gammas:
        seen_acc, unseen_acc, overall = eval_config(w_text, gamma)
        frontier.append(dict(variant=f'sym_wtext{w_text}', gamma=gamma, seen=seen_acc,
                              unseen=unseen_acc, overall_5644=overall))
        print(f'[sym w_text={w_text} gamma={gamma:>4}] seen={seen_acc:.2f} unseen={unseen_acc:.2f} '
              f'overall={overall:.2f}', flush=True)

u_seen0, u_unseen0, u_overall0 = eval_config(0.5, 0)

# conservative-toward-seen pick: among configs with unseen>=50% of oracle_unseen, prefer
# highest seen, tie-break by overall (mirrors task's "be conservative toward not losing seen")
candidates = [f for f in frontier if f['unseen'] >= 0.5 * oracle_unseen_acc]
pool = candidates if candidates else frontier
best = max(pool, key=lambda f: (round(f['overall_5644'], 1), f['seen']))
...
result = dict(
    sanity=dict(seen_only_acc=seen_only_acc, oracle_seen_acc=oracle_seen_acc,
                ...
                passed=gate_ok, detail='symmetric H+L formula (no ft/cap) -- usable uniformly on all 35665 real eval images'),
    ...
    best_compliant=dict(config=f"variant={best['variant']} gamma={best['gamma']}",
                         ...
                         notes='symmetric formula: no ft/cap fine-tuned branches (only cached for '
                               'seen-eval folder); H(+taxon)+0.5L seen-block usable identically on '
                               'every real eval image regardless of folder'),
)
import os
os.makedirs('outputs', exist_ok=True)
json.dump(result, open('outputs/unified_frontier_symmetric.json', 'w'), indent=2)
print('wrote outputs/unified_frontier_symmetric.json')
