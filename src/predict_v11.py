"""v11 = v09 seen route (proto+2cmax+4*taxon-text, FT features) + NEW unseen route:
contrastive-FT image tower @ WiSE-FT scale 0.5 (emb_unseen_ctft05.pt) matched to TAXON text,
ensembled with frozen ViT-L name. First legit unseen gain: hard-sim 23.99 -> 26.32 (+2.33 sim).
  python src/predict_v11.py            # prints seen holdout (~87.2) + projection
  python src/predict_v11.py --generate # writes outputs/prediction_v11.json"""
import json, torch, argparse
from collections import defaultdict
import torch.nn.functional as F
torch.set_num_threads(8); dev="cpu"
LAM = 4.0   # seen taxon-text weight (v09)
...
# CLEAN seen holdout (identical to v09)
...
proj=(20097*real_seen/100 + 15568*0.134)/35665*100   # unseen ~0.134 from FT WiSE route
print(f"SEEN holdout FT-H={a_fth:.2f} FT-H+0.5L={a_ens:.2f} -> {'ENS' if use_ens else 'FTH'}; real seen~{real_seen:.1f}% unseen~13.4% overall~{proj:.1f}%")
...
if A.generate:
    ...
    # NEW unseen route: contrastive-FT H (scale 0.5) @ taxon text + 0.5 frozen-L name
    HunI,HunF,Hunf=load("outputs/emb_unseen_ctft05.pt"); LunI,LunF,_=load("outputs/emb_unseen.pt")
    ...
    json.dump(preds,open("outputs/prediction_v11.json","w"))
    print(f"wrote prediction_v11.json n={len(preds)} test={len(tf)} unseen={len(uf)}")
