"""v83: v79 learned gate + leak-free re-rankers on BOTH heads.

Adds the seen-head re-ranker to v82. The seen head summed proto + 2.0*cmax + 4.0*taxon with
constants applied identically to every class; a learned per-candidate model conditioned on class
frequency gets 89.870 -> 91.674 (+1.803) on class-disjoint CV. No leak is possible on that head:
every one of the 4,636 candidates is gold-eligible for a val_seen image.

Unseen head = v82 (leak-free re-ranker, 59.28 -> 67.43). Gate = v79. f=0.60 unchanged.
...
Seen head, gate (v79) and f=0.60 are unchanged from v79.
  ...
  conda activate onet && python builders/build_v83_rerank_both.py
"""
import json
import os
import pickle
...
RERANK_SEEN_PKL = os.environ.get('RERANK_SEEN_PKL', 'outputs/rerank_seen_v83.pkl')
TAG = os.environ.get('TAG', 'v83_rerank_both_f60')
...
def main():
    print('=== Building v83: v79 gate + re-rankers on both heads (f=0.60 fixed) ===', flush=True)
    need = (CROP_TEST, CROP_UNS, OSTRIP_TEST, OSTRIP_UNS, BANK_CTFT, BANK_336, GATE_PKL,
            RERANK_PKL, RERANK_SEEN_PKL)
    for p in need:
        if not os.path.exists(p):
            raise SystemExit(f'missing {p}')
    gate = pickle.load(open(GATE_PKL, 'rb'))
    rr = pickle.load(open(RERANK_PKL, 'rb'))
    rs = pickle.load(open(RERANK_SEEN_PKL, 'rb'))
    print(f'seen reranker: K={rs["K"]} feats={len(rs["names"])} holdout gain +{rs["gain"]:.3f}', flush=True)
    ...
    # The 10 unseen legs (10x[35665,11598] fp32 = 16.5GB) are built further down, immediately
    # before the unseen re-rank -- the first point they are needed. Building them HERE, as this
    # builder originally did, left 18.2GB of not-yet-needed tensors resident alongside `sig`
    # (9x[35665,5795] = 7.4GB) and OOM'd the gate bank scoring below on a 46GB card.
    ...
    print(f'reranking {len(idx_seen)} seen-route rows (top-{rs["K"]}) ...', flush=True)
    topk_s = seen_block[idx_seen].topk(rs['K'], dim=1).indices
    Xsn, snames = cand_features({n: sig[n][idx_seen] for n in SIGNALS}, topk_s, n_train,
                                seen_block[idx_seen])
    assert snames == rs['names'], 'seen reranker feature definition drifted'
    ps_ = rs['model'].decision_function(
        (Xsn.reshape(-1, len(rs['names'])).cpu().numpy() - rs['mu']) / rs['sd'])
    ...
    print(f'  seen reranker moved {100.0 * (sel_s != 0).float().mean().item():.1f}% off rank-1',
          flush=True)
    del sig
    ...
    del Sraw_ct, Sraw_336  # dbnorm copied them into legs
    ...
    del legs, fused  # dead after slicing; 10x[35665,11598] fp32 = 18.2GB otherwise held through pool_features
