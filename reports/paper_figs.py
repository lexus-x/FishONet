...
def fig_campaign(name="campaign"):
    ...
    top.set_ylim(0, 1)
    top.axis("off")
    # every build that received a leaderboard score (HANDOFF.md ledger tables), not
    # only the 13 milestones plotted beneath; the two sets are drawn differently
    scored = set([22, 23, 25, 27, 28, 31, 33, 34, 36, 37, 39, 40, 41, 43, 44, 46, 50, 56, 63, 77, 79, 81, 82, 83, 84, 86, 87, 99, 109])
    plotted = set(NUM)
    for b in range(1, 110):
        if b in plotted:
            c, lw, h = INK, 1.1, 0.62
        elif b in scored:
            c, lw, h = MUTED, 0.85, 0.62
        else:
            c, lw, h = RULE, 0.6, 0.40
        top.plot([b, b], [0.06, h], color=c, lw=lw, solid_capstyle="butt")
    top.text(0, 0.80, "109 numbered builds", fontsize=5.6, color=MUTED)
    top.text(110, 0.80, f"{len(scored)} scored on the leaderboard · 13 plotted below",
             fontsize=5.6, color=INK, ha="right")
    ...
    ax.text(len(LABELS) - 1, 54.35, "53.761", ha="right", fontsize=9.5,
...
PAIRS = [
    ...
    ("v109 genus backoff",     0.101,  0.064, 1965),
]
