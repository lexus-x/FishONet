...
--shift_aug 1: RandomResizedCrop scale(0.35,1.0) ratio(0.5,2.0) to match test framing
(HANDOFF §3). Pair with squash_tta extract at inference.
"""
...
def main():
    ...
    ap.add_argument('--shift_aug', type=int, default=0,
                    help='1 = test-shift-matched wide-aspect RandomResizedCrop')
    ap.add_argument('--init_ckpt', default='', help='LoRA checkpoint to warm-start')
    a=ap.parse_args()
    if a.init_ckpt:
    ...
    if a.init_ckpt:
        ...
        print(f'init LoRA from {a.init_ckpt} prior_db={ck.get("db")} step={ck.get("step")}', flush=True)
    ...
    # Original recipe used eval preprocess (center crop) for train; shift_aug replaces with
    # wide-aspect RandomResizedCrop. Val always stays center-crop.
    ...
    if a.shift_aug:
        ...
        train_tf = eval_pp
    class DS(Dataset):
        def __init__(s, it, tf): s.it=it; s.tf=tf
        def __len__(s): return len(s.it)
        def __getitem__(s,i):
            fn,y=s.it[i]
            try: return s.tf(Image.open(imgidx[fn]).convert('RGB')),y
            except: return torch.zeros(3,224,224),y
    dl=DataLoader(DS(train_items, train_tf),batch_size=a.bs,shuffle=True,num_workers=a.workers,pin_memory=True,drop_last=True)
