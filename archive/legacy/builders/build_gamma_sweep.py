"""Gamma sweep for the compliant ctftbig single pipeline.
Reuses build_v21_unified_submission_ctftbig.py's heavy compute (embeddings, seen_block,
text_block, novelty) ONCE, then writes one submission zip per (w_text, gamma) config.
Goal: find the real-scorer optimum -- gamma=20 measured real seen71.92/unseen10.85/overall45.26,
so bracket it to recover seen (seen weight 0.564)."""
...
seen_set = set(seen)
CONFIGS = [(0.5, 10.0), (0.5, 13.0), (0.5, 16.0), (0.5, 20.0), (0.5, 24.0)]
for W_TEXT, GAMMA in CONFIGS:
    U = text_block.clone()
    ...
    tag = f'v21_g{int(GAMMA)}_w{W_TEXT}'
