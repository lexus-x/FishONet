"""v48: Corrected 2-Stage Soft-Gated Unified Inference Pipeline Builder.

Calibrates seen prototype z-scores and unseen text z-scores on matching scales:
  - sb_z = row_z(seen_block)
  - tu_z = row_z(text_unseen_only)
  - novelty prior boost shifts unseen logits on novel images, yielding balanced predictions.

Outputs zip packages ready for Codabench upload:
  - submissions/submission_v48_2stage_k10.zip
  - submissions/submission_v48_2stage_k50.zip
  - submissions/submission_v48_2stage_k100.zip
"""
import json
import os
import pickle
...
def main():
    print("=== Building Scale-Calibrated v48 2-Stage Soft-Gated Submission ===", flush=True)
    ...
    # Stage 1 Vision-Prototype Scores for Seen Classes
    ...
    # Row-standardized scores for consistent matching scale
    sb_z = row_z(seen_block)
    tu_z = row_z(text_unseen_only)

    # Gating Novelty Signal for continuous calibration
    ...
    gate_signal = z1(img_seenmax) + 2.0 * z1(text_margin)
    nov_prior = torch.sigmoid(-gate_signal / 1.0) # High for novel/unseen images

    os.makedirs('submissions', exist_ok=True)
    seen_set = set(seen)

    for k_top in [10, 50, 100]:
        print(f"\n--- Generating Submission Package for k_top={k_top} ---")
        topk_local = seen_block.topk(k_top, dim=1).indices
        
        logits = torch.full((len(all_files), NCLS), -1e9, device=dev)
        
        # Set unseen class logits with novelty boost
        logits[:, other_idx] = tu_z + 4.0 * nov_prior.unsqueeze(1)
        
        for i in range(len(all_files)):
            sel_seen = kept_idx[topk_local[i]]
            logits[i, sel_seen] = sb_z[i, topk_local[i]]
