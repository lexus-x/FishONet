"""v109: v83 + genus-level hierarchical backoff on the seen route (the "gamble" submission).

Holdout stack test (research/genus_recall10_v103.py + research/genus_rerank_v103_train.py):
genus bonus (ctftshift-leg proto+2*cmax pooled ACROSS sibling species of the same genus,
LAM_G=3.0) added to the deployed 3-leg seen fusion improves candidate-pool recall@10
(98.854 -> 99.031) and, after retraining the seen re-ranker leak-free with genus as a 4th
signal group, reaches 91.775% holdout stack accuracy vs the deployed v83 stack's 91.674%
(real-scored) -- a +0.101pt edge. This is INSIDE the noise band the exact same axis already
demonstrated today (v98/v99: +1.34pt holdout -> -0.17pt real), so expected value is roughly
flat-to-negative. Shipped anyway per explicit instruction ("gamble it").

Isolation design (same pattern as v99): GATE and UNSEEN route are bit-identical to v83 --
only the seen-route candidate SET (genus-augmented) and the seen re-ranker (retrained with
the genus signal, outputs/rerank_seen_genus_v103.pkl) change.

  conda activate onet && python builders/build_v109_genus_gamble.py
"""
import json
import os
import pickle
...
TAG = os.environ.get('TAG', 'v109_genus_gamble')
