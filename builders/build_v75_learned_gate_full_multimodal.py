"""v75: Inductive Learned Logistic Soft Gate + Full Multimodal Unseen Head.
...
1. Gating Mechanism (100% Learning-Based, Inductive O(1)):
   ...
   - Bayes-optimal decision threshold: P(seen | x) >= 0.50 -> Seen Head, else -> Unseen Head.

2. Seen Candidate Head:
   - Multi-encoder seen ensemble (ftshift: 2.5, fullft336shift: 2.5, ctftshift: 1.0).
   - Prototypes + instance similarity + taxonomic text guidance.

3. Multimodal Unseen Candidate Head (Max-Capacity Retrieval):
...
Usage:
  /home/ubuntu/miniconda3/envs/onet/bin/python builders/build_v75_learned_gate_full_multimodal.py
"""
...
def main():
    t0 = time.time()
    print("=" * 85)
    print("=== v75: Learned Logistic Soft Gate + Full Multimodal Unseen Head ===")
    print("=" * 85)
    ...
    # 6. Learned Logistic Soft Gate (100% Inductive, O(1), Zero Batch-Quantiles)
    print('\nEvaluating Learned Logistic Soft Gate (Inductive Inference)...', flush=True)
    ...
    print(f'  ├─ Decision Rule    : P(seen | x) >= 0.50 (Inductive, per-sample)')
    ...
    out_json = os.path.join(OUT, 'prediction_v75_learned_gate_full_multimodal.json')
    with open(out_json, 'w') as f:
        json.dump(preds, f, indent=2)
    print(f'  ├─ Saved JSON: {out_json}')

    out_zip = os.path.join(SUB, 'submission_v75_learned_gate_full_multimodal.zip')
