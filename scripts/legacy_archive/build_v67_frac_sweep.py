"""v67: Optimal Seen-Fraction Sweep with v56's Proven Recipe.

Sweeps SEEN_FRAC from 0.55 to 0.80 and generates the fraction that maximizes
the weighted overall = 0.5635 * seen_acc + 0.4365 * unseen_acc on proxy estimates.

The key insight: each point of seen accuracy is worth 3.96x more than each point
of unseen accuracy. v56 used f=0.60 but this may not be optimal.

Usage:
  /home/ubuntu/miniconda3/envs/onet/bin/python scripts/build_v67_frac_sweep.py
"""
import json
import os
import pickle
...
def main():
    print("=" * 85)
    print("=== Building v67: Optimal Seen-Fraction Sweep ===")
    print("=" * 85)
    ...
    print("Computing unseen retrieval...")
    ...
    # Routing signal (v56's proven formula)
    ...
    # Sweep SEEN_FRAC
    print("\n--- Sweeping SEEN_FRAC ---")
    print(f"{'f':>6} | {'SeenRoute':>10} | {'Test→Seen':>10} | {'Uns→Novel':>10} | {'Proj_Test':>10} | {'Proj_Uns':>10} | {'Proj_Overall':>12}")
    print("-" * 85)
    ...
    # Based on v65 real measurements: specialist_acc ≈ 87.1%, retrieval_acc ≈ 28.4%
    SPEC_ACC = 0.871
    RETR_ACC = 0.284

    for frac in [0.55, 0.58, 0.60, 0.62, 0.64, 0.66, 0.68, 0.70, 0.72, 0.75, 0.78, 0.80]:
        k_seen = int(round(frac * Nq))
        ...
        # Proxy projection using v65's measured specialist/retrieval accuracy
        proj_test = test_seen_rate * SPEC_ACC
        proj_uns = uns_novel_rate * RETR_ACC
        proj_overall = 0.5635 * proj_test + 0.4365 * proj_uns

        print(f"{frac:>6.2f} | {route_seen.sum().item():>10,} | {100*test_seen_rate:>9.1f}% | "
              f"{100*uns_novel_rate:>9.1f}% | {100*proj_test:>9.1f}% | {100*proj_uns:>9.1f}% | "
              f"{100*proj_overall:>11.2f}%")

        if proj_overall > best_proj:
            best_proj = proj_overall
            best_frac = frac
            best_preds = preds_tmp

    print(f"\n>>> Best fraction: f={best_frac:.2f} (projected overall: {100*best_proj:.2f}%)")

    # Also generate at the best fraction
    preds = best_preds
