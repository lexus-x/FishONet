"""v70: Omni-Visual Unseen Bank + 82.2% Seen Specialist.

Integrates:
1. Seen Specialist (82.2% top-1 capability):
   - ctftshift (w=1.0)
   - ftshift (w=2.5)
   - fullft336shift (w=2.5)
   - h-space taxon text guidance (LAM=4.0)

2. Multi-Bank Unseen Retrieval (validated improvements on holdout):
   - CTFT crop-views bank (w=4.0)
   - FullFT336 photo bank (w=3.0)
   - FTShift photo bank (w=1.0)
   - BioCLIP-2 Frozen prototypes (w=2.5)
   - BioCLIP-2 LoRA prototypes (w=2.0)
   - Multi-text embedding fusion (ctft + L + fullft336 + ftshift + promptens + taxabind)
   - Sinkhorn balancing (tau=1.8)

3. Precision Routing (SEEN_FRAC = 0.62 with selective 10% high-confidence seen rescue)

Usage:
  /home/ubuntu/miniconda3/envs/onet/bin/python scripts/build_v70_omni_bank.py
"""
import json
import os
import pickle
...
def main():
    print("=" * 85)
    print("=== Building v70: Omni-Visual Unseen Bank + 82.2% Seen Specialist ===")
    print("=" * 85)
    ...
    # Load all evaluation embeddings
    ...
    tf = sorted(set.intersection(*[set(f) for (_, _, f) in test.values()],
                                 set(tb_test[2]), set(b2_test[2])))
    uf = sorted(set.intersection(*[set(f) for (_, _, f) in unseen_emb.values()],
                                 set(tb_uns[2]), set(b2_uns[2])))
    all_files = tf + uf
    ...
    # 1. SEEN SPECIALIST
    print("Computing seen specialist (82.2% top-1 formulation)...")
    seen_block = torch.zeros(Nq, S, device=dev)
    for t, hs, w in MEMBERS:
        if w == 0.0: continue
        seen_block = seen_block + w * zc(seen_score(Q[t], *PR[t], hs))

    # 2. UNSEEN RETRIEVAL HEAD (Omni-Visual Fusion)
    print("Computing unseen retrieval head...")
    ...
    print("  Scoring CTFT crop-views bank (w=4.0)...")
    ...
    print("  Scoring FullFT336 photo bank (w=3.0)...")
    Sraw_336 = score_bank(Q['fullft336shift'], torch.load(BANK_336, weights_only=False)['bank'], other_list)

    print("  Scoring FTShift photo bank (w=1.0)...")
    Sraw_ft = score_bank(Q['ftshift'], torch.load(BANK_FT, weights_only=False)['bank'], other_list)

    # BioCLIP-2 Proto Legs
    ...
    img_leg = W_CTFT * dbnorm(Sraw_ct) + W_336 * dbnorm(Sraw_336) + W_FT * dbnorm(Sraw_ft)
    del Sraw_ct, Sraw_336, Sraw_ft; torch.cuda.empty_cache()

    text_unseen_only = (text_full[:, other_idx] + img_leg + 
                        B2_WF * b2_frozen_leg + B2_WL * b2_lora_leg)
    del text_full, img_leg, b2_frozen_leg, b2_lora_leg; torch.cuda.empty_cache()

    # 3. ROUTING DECISION WITH RESCUE
    ...
    # Rescue top 10% highest seen-confidence images from unseen route
    base_unseen_idx = (~route_seen).nonzero(as_tuple=True)[0]
    seen_conf = seen_block.max(dim=1).values
    ...
    if len(idx_uns) > 0:
        print(f"Applying Sinkhorn on {len(idx_uns):,} unseen predictions...")
    ...
    print(f"\nFinal Summary (v70 Omni-Visual Unseen Bank + 82.2% Seen Specialist):")
    print(f"  ├─ Total Predictions: {len(preds):,}")
    print(f"  ├─ Unique Unseen Classes: {cov:,}/{Cu:,}")
    print(f"  ├─ Test Folder -> Seen Rate: {100*(1-o_t/len(tf)):.2f}%")
    print(f"  └─ Unseen Folder -> Novel Rate: {100*o_u/len(uf):.2f}%")
    ...
    print(f"\n[SUCCESS] Saved submission: {sub_path} ({os.path.getsize(sub_path):,} bytes)")
    print("=" * 85)
