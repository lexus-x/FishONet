"""Real submission builder: legitimate content-based hard routing (seenmax ensemble
signal, threshold calibrated on holdout at thr_frac=0.550) + 4-channel seen_block
(ft+cap+L+fullft336). No folder/path leakage -- route decided purely from each real
image's own embedding, exactly like unified_holdout_ctftbig_hardroute.py, just run over
the real 35,665-image batch instead of the holdout split.
...
Holdout showed seen=83.19 unseen=21.53 overall=56.30 vs the smooth-gate reference's
seen=88.99 unseen=4.06 overall=51.96 at the same seen_block. Real transfer is UNVERIFIED.
"""
...
print(f'seen classes={S}  other_idx(NCLS-S)={len(other_idx)}')
...
seen_block = zc(sc_ft) + 1.0 * zc(sc_cap) + 0.5 * zc(sc_l) + 0.75 * zc(sc_q)

M_taxon = qB.to(dev) @ TtH.t()
M_name = qL.to(dev) @ TnL.t()
text_dbnorm_full = dbnorm(M_taxon, 0.05, 0.5) + 0.5 * dbnorm(M_name, 0.05, 0.5)
text_unseen_only = text_dbnorm_full[:, other_idx]

seenmax = seen_block.max(1).values
thr_frac = 0.550   # calibrated on holdout (unified_holdout_ctftbig_hardroute.py)
thr = seenmax.min().item() + thr_frac * (seenmax.max().item() - seenmax.min().item())
route_seen = seenmax >= thr
...
print(f'routed_seen={n_route_seen}/{len(all_files)} ({100*n_route_seen/len(all_files):.1f}%)')
