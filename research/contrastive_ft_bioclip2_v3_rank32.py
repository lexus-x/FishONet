"""BioCLIP-2 LoRA v3 rank32 (scratch taxctx, longer): frozen taxctx text (768-d) + optional disclosed iNat mix.

Warm-start from v1 LoRA or scratch. Longer schedule for encoder lift toward v41 stack.

  conda activate onet
  python research/contrastive_ft_bioclip2_v2.py --shift_aug 1 --max_steps 2000 \\
      --init_ckpt outputs/b2_shift_lora.pt --inat 1 \\
      --save outputs/b2_shift_lora_v2.pt 2>&1 | tee outputs/b2_shift_lora_v2_train.log
"""
import argparse
import json
import math
...
def main():
    ...
    ap.add_argument('--save', default='outputs/b2_shift_lora_v3_rank32.pt')
    a = ap.parse_args()
    ...
    if a.init_ckpt and os.path.isfile(a.init_ckpt):
        ...
        print(f'LoRA init (rank fixed={a.rank}): alpha={a.alpha} top_k={a.top_k}', flush=True)
    ...
    while not done:
        for x, y in dl:
            ...
            with torch.autocast('cuda', dtype=torch.bfloat16):
                f = F.normalize(model.encode_image(x).float(), dim=-1)
                loss = F.cross_entropy(a.temp * (f @ Ttrain.t()), y)
            opt.zero_grad()
            loss.backward()
            opt.step()
            step += 1
            if step % 50 == 0:
                print(f'step {step}/{a.max_steps} loss={loss.item():.3f} {(time.time()-t0)/step:.2f}s/it',
                      flush=True)
