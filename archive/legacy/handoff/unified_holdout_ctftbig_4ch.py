"""Test full-FT-336 (fullft336, trained+extracted 2026-07-15/16, standalone val NCM=83.60%,
never combined into the ensemble) as a 4th DECORRELATED seen-block channel, mirroring
how 'cap' was added as v20's 3rd channel (+0.72 holdout, +0.74 real -- the project's one
confirmed-real seen-side win). Copy of unified_holdout_ctftbig.py; only the seen_block
gains a 4th zc(sc_fullft336) term with a swept weight. Everything else identical.
"""
...
sc_q = seen_score(qQ, PQ, TFQ, TLQ)   # fullft336, no taxon-text term (not trained for text alignment)
...
print(f'BASELINE (ft+1.0cap+0.5L) oracle_seen={base_acc:.2f}  SE={se:.2f}  n={n_seen_q}')
...
for w_q in [0.25, 0.5, 0.75, 1.0, 1.5]:
    ...
    print(f'[+fullft336 w={w_q}] oracle_seen={acc:.2f}  delta={acc-base_acc:+.2f}  SE={se:.2f}')
...
json.dump(dict(baseline=base_acc, se=se, n_seen_q=n_seen_q, results=results, best=best),
          open('handoff/unified_frontier_4ch_fullft336.json', 'w'), indent=2)
print('wrote handoff/unified_frontier_4ch_fullft336.json')
