"""Alternative novelty-gate SIGNALS for the compliant ctftbig single pipeline.
Current gate uses raw max ft-prototype cosine (maxproto), which the project found barely
separates seen vs unseen (0.913 vs 0.898). Try stronger seen-confidence signals derived from
the full ENSEMBLE seen_block instead:
  - seenmax : max of the ensembled seen_block score  (stronger than raw ft-proto cosine)
  - energy  : logsumexp of seen_block (total seen mass) -- classic OOD energy score
Each novelty = 1 - minmax(signal); offset = gamma*novelty on seen cols; one argmax.
Reuses build_v21_unified_submission_ctftbig.py's heavy compute once."""
...
def minmax(v): return (v - v.min()) / (v.max() - v.min() + 1e-6)
...
kept_idx = torch.tensor([ci[c] for c in seen]).to(dev); TseenTax = TtH[kept_idx]
...
FteI, FteF, Ftef = load('outputs/emb_test_ft.pt'); CteI, CteF, _ = load('outputs/emb_test_cap.pt')
LteI, LteF, _ = load('outputs/emb_test.pt'); HteI, _, Htef = load('outputs/emb_test_h.pt')
BteI, BteF, _ = load('outputs/emb_test_ctftbig.pt')
tf = sorted(fn for fn in Htef if fn in FteI and fn in CteI and fn in LteI)
FunI, FunF, _ = load('outputs/emb_unseen_ft.pt'); CunI, CunF, _ = load('outputs/emb_unseen_cap.pt')
LunI, LunF, _ = load('outputs/emb_unseen.pt'); HunI, _, Hunf = load('outputs/emb_unseen_h.pt')
...
M_taxon = qB.to(dev) @ TtH.t(); M_name = qL.to(dev) @ TnL.t()
...
seen_block = zc(sc_ft) + 1.0 * zc(sc_cap) + 0.5 * zc(sc_l)   # [N,S]

SIGNALS = {
    'seenmax': 1.0 - minmax(seen_block.max(1).values),
    'energy':  1.0 - minmax(torch.logsumexp(seen_block, dim=1)),
}
seen_set = set(seen); W_TEXT = 0.5
for sig_name, novelty in SIGNALS.items():
    for GAMMA in [13.0, 20.0]:
        U = text_block.clone()
        ...
        tag = f'v21_{sig_name}_g{int(GAMMA)}'
        json.dump(preds, open(f'outputs/prediction_{tag}.json', 'w'))
        z = zipfile.ZipFile(f'outputs/submission_{tag}.zip', 'w', zipfile.ZIP_DEFLATED); z.write(f'outputs/prediction_{tag}.json', arcname='prediction.json'); z.close()
        print(f'{tag}: test->other {100*o_t/len(tf):.1f}%  unseen->other {100*o_u/len(uf):.1f}%  wrote submission_{tag}.zip')
print('DONE (none uploaded)')
