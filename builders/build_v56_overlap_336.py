"""v56: v50 stack + max(center, squash, 5 overlapping long-axis strips) + 336 iNat bank @ w=3.

Holdout crop_chase53_proxy: this recipe +1.424 vs v50-like (best).
center/squash from emb_*_ctft_cropviews.pt; strips from emb_*_ctft_ostrip.pt.
Single pipeline, f=0.60, tau=1.8, argmax over full 17393 classes.

  conda activate onet && python builders/build_v56_overlap_336.py
"""
import json
import os
import pickle
...
def main():
    print('=== Building v56 overlap-strip crop-max + 336 iNat bank w=3 ===', flush=True)
    for p in (CROP_TEST, CROP_UNS, OSTRIP_TEST, OSTRIP_UNS, BANK_CTFT, BANK_336):
