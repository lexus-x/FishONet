"""v41: v40 + frozen BioCLIP-2 (imageomics/bioclip-2 ViT-L/14) iNat mean-proto leg on UNSEEN route.
...
TAXABIND_W = 1.0
IMG_WEIGHTS = [float(x) for x in os.environ.get('IMG_WEIGHTS', '4,3').split(',')]
TOPM = int(os.environ.get('TOPM', '4'))
BANK_PATH = os.environ.get('BANK_PATH', 'outputs/inat_photo_bank_ctftshift.pt')
B2_W = float(os.environ.get('B2_W', '2.5'))
B2_PROTO = os.environ.get('B2_PROTO', 'outputs/inat_protos_bioclip2.pt')
...
b2_test = load('outputs/emb_test_bioclip2.pt')
b2_uns = load('outputs/emb_unseen_bioclip2.pt')
Qb2 = torch.cat([
...
for IMG_W in IMG_WEIGHTS:
    ...
    tag = f'v41_bioclip2proto_w{IMG_W:g}_b2{B2_W:g}_sink72'
