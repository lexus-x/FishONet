...
def main():
    ...
    ap.add_argument('--top_k',type=int,default=4); ap.add_argument('--rank',type=int,default=16); ap.add_argument('--alpha',type=int,default=32)
    ap.add_argument('--bs',type=int,default=48); ap.add_argument('--lr',type=float,default=5e-4)
    ap.add_argument('--max_steps',type=int,default=800); ap.add_argument('--val_every',type=int,default=100)
    ap.add_argument('--workers',type=int,default=4); ap.add_argument('--temp',type=float,default=30.0); ap.add_argument('--val_imgs',type=int,default=2318)
    ap.add_argument('--save',default='outputs/ctft_cleanse.pt')
    a=ap.parse_args()
    D='data/dl'
    ...
    train_items=[(fn,k2i[lab[fn]]) for c in known for fn in (sorted(by[c])[:-max(1,round(0.2*len(by[c])))] if len(by[c])>=3 else sorted(by[c]))]
