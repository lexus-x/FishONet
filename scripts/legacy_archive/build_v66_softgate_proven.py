"""v66: Soft-Gated Routing + Proven Unseen Retrieval.

Combines:
- v63's learned logistic novelty gate (AUC 0.9559) for ROUTING
- v61's seen specialist formulation (81.90% real accuracy)  
- v56's proven dbnorm + sinkhorn unseen retrieval (20.57% real accuracy)

The key insight: v61's gate correctly identifies seen images (81.9% test accuracy)
but its unseen head was broken (wrong fusion weights → 6.85% unseen).
v56's unseen head works (20.57%) but its hard-fraction routing is suboptimal.
v66 marries the two: learned gate routes → proven unseen retrieves.

Target: 0.5635 × 81% + 0.4365 × 20.5% = 55.6%

Usage:
  /home/ubuntu/miniconda3/envs/onet/bin/python scripts/build_v66_softgate_proven.py
"""
import json
import math
import os
...
def main():
    print("=" * 85)
    print("=== Building v66: Soft-Gated Routing + Proven Unseen Retrieval ===")
    print("=" * 85)
    ...
    # v56's proven seen ensemble weights (validated on real leaderboard)
    MEMBERS = [
        ('ctftshift', True, 1.0),
        ('ftshift', True, 2.5),
    ...
    print(f'Eval: {len(all_files):,} (Test: {len(tf):,}, Unseen: {len(uf):,})')
    ...
    # SEEN HEAD (v56's proven weights)
    print("Seen specialist (v56 weights)...")
    ...
    # UNSEEN HEAD (v56's proven dbnorm+sinkhorn recipe)
    print("Unseen retrieval (v56 dbnorm + sinkhorn)...")
    ...
    # ============================================
    # ROUTING: v63's LEARNED logistic novelty gate
    # ============================================
    print("Computing learned logistic novelty gate routing...")
    ...
    # Sweep over routing thresholds to find optimal seen fraction
    best_tag = None
    best_proj = -1.0

    for thr_name, route_seen in [
        # Option A: Learned gate at optimal threshold
        ("gate_opt", p_seen >= gate.optimal_threshold),
        # Option B: Learned gate at 0.50
        ("gate_50", p_seen >= 0.50),
        # Option C: Learned gate calibrated to match v56's 60% seen fraction
        ("gate_f60", p_seen >= torch.topk(p_seen, int(round(0.60 * Nq))).values.min()),
        # Option D: v56's original hard routing (as baseline comparison)
        ("hard_f60", None),
        # Option E: Gate at 0.70 (more aggressive novel routing)
        ("gate_70", p_seen >= 0.70),
    ]:
        if route_seen is None:
            # v56's original routing
            ...
            k_seen = int(round(0.60 * Nq))
            thr_val = torch.topk(combined, k_seen).values.min()
            route_seen = combined >= thr_val
        ...
        seen_rate = 1 - o_t / len(tf)
        unseen_rate = o_u / len(uf)
        n_seen_route = route_seen.sum().item()

        print(f"  [{thr_name}] Seen route: {n_seen_route:,}/{Nq:,} ({100*n_seen_route/Nq:.1f}%) | "
              f"Test→Seen: {100*seen_rate:.1f}% | Unseen→Novel: {100*unseen_rate:.1f}%")

        if thr_name == "gate_f60":
            best_tag = thr_name
            best_preds = preds_tmp
            best_route = route_seen.clone()

    # Use gate_f60 as best (matches v56's proven 60% fraction but with learned gate)
    preds = best_preds
