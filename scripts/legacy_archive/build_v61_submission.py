"""Build v61: Soft Marginal Probabilistic Fusion + Query-Centered Degree-3 Polynomial Calibrators.

Implements the superior architecture:
1. Continuous Soft Probabilistic Marginal Fusion:
   log P(c | x) = log P(seen | x) + log Softmax(S_seen / tau_s)  [seen]
   log P(c | x) = log(1 - P(seen | x)) + log Softmax(S_unseen / tau_u)  [unseen]
2. Query-Centered Monotonic Degree-3 Polynomial Calibration (100% inductive, zero dbnorm).
3. Dynamic Confidence Attention Modality Fusion across 7-view cropmax photo bank, 336 photo bank, BioCLIP-2 ToL protos, and text embeddings.
4. Single-pipeline compliant: one uniform rule over all 35,665 images, full 17,393 argmax.

Usage:
  /home/ubuntu/miniconda3/envs/onet/bin/python scripts/build_v61_submission.py
"""
...
def query_center_poly(S: torch.Tensor, poly: MonotonicDegree3Polynomial) -> torch.Tensor:
    """Standardizes per query (row-wise) and applies monotonic degree-3 polynomial."""
...
class DynamicConfidenceFusion(nn.Module):
    ...
    def forward(self, legs: list[torch.Tensor]) -> torch.Tensor:
        ...
        return (stacked * weights).sum(dim=-1) # [N, C]


def main():
    print("=" * 80)
    print("=== Building v61: Soft Marginal Probabilistic Fusion Submission (No DBNorm) ===")
    print("=" * 80)
    ...
    print("Computing Raw Unseen Scores Across Legs & Applying Degree-3 Calibration...")
    ...
    # Accumulate directly into unseen_scores
    ...
    unseen_scores += 0.35 * query_center_poly(Sraw_ctft, poly_ctft)
    ...
    unseen_scores += 0.25 * query_center_poly(Sraw_336, poly_336)
    del Sraw_336
    torch.cuda.empty_cache()
    ...
    unseen_scores += 0.15 * query_center_poly(Sraw_b2_f, poly_b2f)
    ...
    unseen_scores += 0.15 * query_center_poly(Sraw_b2_l, poly_b2l)
    del Sraw_b2_l, Qb2l, p_b2_l
    torch.cuda.empty_cache()

    # 5. Text Taxon
    Sraw_txt_taxon = Q['ctftshift'] @ TtH[other_idx].t()
    unseen_scores += 0.05 * query_center_poly(Sraw_txt_taxon, poly_taxon)
    del Sraw_txt_taxon
    torch.cuda.empty_cache()

    # 6. Text TaxaBind
    Sraw_txt_tb = Qtb @ Ttb[other_idx].t()
    unseen_scores += 0.05 * query_center_poly(Sraw_txt_tb, poly_tb)
    del Sraw_txt_tb, Qtb
    torch.cuda.empty_cache()

    # 4. Extract Novelty Gating Features & Compute P(seen | x)
    print("Estimating Calibrated P(seen | x) via Logistic Regression Gate...")
    ...
    pkg = torch.load(os.path.join(OUT, 'learned_pipeline_v60.pt'), map_location='cpu', weights_only=False)
    ...
    # 5. Continuous Soft Marginal Probabilistic Fusion
    print("Computing Joint Marginal Log-Likelihoods over Full 17,393 Classes...")
    tau_seen = 1.80
    tau_unseen = 1.65
    ...
    # Log-probabilities over seen classes [Nq, S]
    log_prob_seen = log_p_seen + F.log_softmax(seen_block / tau_seen, dim=1)

    # Log-probabilities over unseen classes [Nq, Cu]
    log_prob_unseen = log_p_unseen + F.log_softmax(unseen_scores / tau_unseen, dim=1)

    # Combined full logit space [Nq, S + Cu] = [35665, 17393]
    full_log_probs = torch.cat([log_prob_seen, log_prob_unseen], dim=1)
    full_argmax = full_log_probs.argmax(dim=1)

    # Map argmax index to global class index
    ...
    print(f"\nPrediction Summary (v61 Soft Marginal):")
    ...
    print(f"\n[SUCCESS] Generated v61 Submission Package:")
