...
TAG = os.environ.get('TAG', 'v109_compliant_dbnorm')
...
def dbnorm_frozen(S, h, tc=0.05, tr=0.5):
    """Per-image: the dim=0 (across-image) term is replaced by a constant fit once on a
    reference pool, never on the eval batch. dim=1 (across-class, within THIS image) stays
    live -- that part never depended on other images."""
...
def main():
    ...
    # ---- frozen dbnorm hub offsets: estimate the per-class across-image bias h_c on a
    # REFERENCE pool disjoint from the eval batch (held-out seen-class crop-views, same
    # methodology as research/dbnorm_frozen_hub.py), freeze it, so the unseen-route legs
    # below no longer read the live 35,665-image eval batch at all -- only this image's own
    # embedding plus a constant.
    HUB_TC = 0.05

    def hub_offset(Sref):
        return torch.logsumexp(Sref / HUB_TC, dim=0)

    hold = torch.load('archive/pre_v50/outputs/emb_holdout_ctft_cropviews_v2.pt', weights_only=False)
    common_set = set(common)
    ref_files = [fn for fn in hold['files'] if fn in common_set]
    hidx = {fn: i for i, fn in enumerate(hold['files'])}
    ref_views = {k: F.normalize(hold['views'][k].float(), dim=-1)[[hidx[fn] for fn in ref_files]].to(dev)
                 for k in VIEW_KEYS if k in hold['views']}
    print(f'hub reference pool: {len(ref_files)} held-out seen-class images (label-disjoint '
          f'from the {len(other_idx)} novel classes by construction)', flush=True)

    def ref_emb_from(idx, feats):
        return torch.stack([feats[idx[fn]] for fn in ref_files]).to(dev)

    R = {t: ref_emb_from(train[t][0], train[t][1]) for t, _, _ in MEMBERS}
    Rtb_idx, Rtb_feats, _ = load('outputs/emb_train_taxabind.pt')
    Rtb = ref_emb_from(Rtb_idx, Rtb_feats)
    del Rtb_idx, Rtb_feats
    H = {
        't_ctft_taxon': hub_offset(R['ctftshift'] @ TtH[other_idx].t()),
        't_L_name': hub_offset(R['L'] @ TnL[other_idx].t()),
        't_336v2_taxon': hub_offset(R['fullft336_v2'] @ TtH[other_idx].t()),
        't_ftshift_taxon': hub_offset(R['ftshift'] @ TtH[other_idx].t()),
        't_ctft_taxctx': hub_offset(R['ctftshift'] @ TTX[other_idx].t()),
        't_taxabind': hub_offset(Rtb @ Ttb[other_idx].t()),
    }
    print('text-leg hub offsets frozen from reference pool', flush=True)
    ...
    Nq = len(all_files)
    Cu = len(other_list)
    bank_ct_dict = torch.load(BANK_CTFT, weights_only=False)['bank']
    bank_336_dict = torch.load(BANK_336, weights_only=False)['bank']

    # frozen hub offsets for the remaining 4 legs (bank + b2), same reference pool
    ref_view_list = [ref_views[k] for k in VIEW_KEYS if k in ref_views]
    H['bank_ct'] = hub_offset(score_bank_maxviews(ref_view_list, bank_ct_dict, other_list))
    H['bank_336'] = hub_offset(score_bank(R['fullft336shift'], bank_336_dict, other_list))

    def b2_ref_offset(proto_path, emb_train_path):
        idx, feats, _ = load(emb_train_path)
        Rb2 = ref_emb_from(idx, feats)
        b2P = F.normalize(torch.load(proto_path, weights_only=False)['protos'].float(), dim=-1).to(dev)
        Sr = torch.full((Rb2.shape[0], Cu), -1e4, device=dev)
        for j, gidx in enumerate(other_list):
            pvec = b2P[gidx]
            if pvec.norm() >= 0.5:
                Sr[:, j] = Rb2 @ pvec
        return hub_offset(Sr)

    H['b2_frozen'] = b2_ref_offset(B2_PROTO_FROZEN, 'outputs/emb_train_bioclip2.pt')
    H['b2_lora'] = b2_ref_offset(B2_PROTO_LORA, 'outputs/emb_train_bioclip2_lora_v2.pt')
    del R, Rtb, ref_views, ref_view_list
    print(f'all 10 hub offsets frozen: {sorted(H.keys())}', flush=True)

    bank_ct = bank_ct_dict
    ...
    Sraw_336 = score_bank(Q['fullft336shift'], bank_336_dict, other_list)

    def b2_leg(proto_path, emb_test_path, emb_uns_path, h):
        ...
        for j, gidx in enumerate(other_list):
            ...
            if pvec.norm() < 0.5:
                continue
            Sraw_b[:, j] = Qb2 @ pvec
        return dbnorm_frozen(Sraw_b, h), Qb2

    b2_frozen_leg, Qb2f = b2_leg(B2_PROTO_FROZEN, 'outputs/emb_test_bioclip2.pt',
                                  'outputs/emb_unseen_bioclip2.pt', H['b2_frozen'])
    b2_lora_leg, Qb2l = b2_leg(B2_PROTO_LORA, 'outputs/emb_test_bioclip2_lora_v2.pt',
                                'outputs/emb_unseen_bioclip2_lora_v2.pt', H['b2_lora'])
    ...
    legs = {
        't_ctft_taxon': dbnorm_frozen(Q['ctftshift'] @ TtH[other_idx].t(), H['t_ctft_taxon']),
        't_L_name': dbnorm_frozen(Q['L'] @ TnL[other_idx].t(), H['t_L_name']),
        't_336v2_taxon': dbnorm_frozen(Q['fullft336_v2'] @ TtH[other_idx].t(), H['t_336v2_taxon']),
        't_ftshift_taxon': dbnorm_frozen(Q['ftshift'] @ TtH[other_idx].t(), H['t_ftshift_taxon']),
        't_ctft_taxctx': dbnorm_frozen(Q['ctftshift'] @ TTX[other_idx].t(), H['t_ctft_taxctx']),
        't_taxabind': dbnorm_frozen(Qtb @ Ttb[other_idx].t(), H['t_taxabind']),
        'bank_ct': dbnorm_frozen(Sraw_ct, H['bank_ct']),
        'bank_336': dbnorm_frozen(Sraw_336, H['bank_336']),
