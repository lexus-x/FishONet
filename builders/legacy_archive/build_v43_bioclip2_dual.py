"""v43: v41 stack + dual BioCLIP-2 mean-proto legs on UNSEEN route.
...
B2_WF = float(os.environ.get('B2_WF', '0.5'))
B2_WL = float(os.environ.get('B2_WL', '1.0'))
B2_PROTO_FROZEN = os.environ.get('B2_PROTO_FROZEN', 'outputs/inat_protos_bioclip2.pt')
...
print(f'B2 dual proto: frozen cols {ncov_f}/{Cu} LoRA cols {ncov_l}/{Cu}; wf={B2_WF} wl={B2_WL}', flush=True)
...
for IMG_W in IMG_WEIGHTS:
    ...
    tag = f'v43_b2dual_w{IMG_W:g}_wf{B2_WF:g}_wl{B2_WL:g}_sink72'
