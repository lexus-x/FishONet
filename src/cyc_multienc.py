...
FtI,FtF,Ftf=load('outputs/emb_train_ft.pt')   # v15 FT-H (deployed)
CtI,CtF,_=load('outputs/emb_train_cap.pt')     # capacity FT-H
LtI,LtF,_=load('outputs/emb_train.pt')         # L
...
base=a(zc(sF)+0.5*zc(sL))
print(f"=== v15 baseline (ft + 0.5L): {base:.2f}%  (should ~= 87.22) ===",flush=True)
print(f"cap alone (cap + 0.5L): {a(zc(sC)+0.5*zc(sL)):.2f}%",flush=True)
best=(base,"v15 baseline")
for w in [0.25,0.5,0.75,1.0,1.25,1.5,2.0]:
    x=a(zc(sF)+w*zc(sC)+0.5*zc(sL))
    print(f"  ft + {w:.2f}*cap + 0.5L -> {x:.2f}  (delta {x-base:+.2f})",flush=True)
    if x>best[0]: best=(x,f"ft+{w}cap+0.5L")
print(f"CYCLE2_RESULT best={best[0]:.2f}% [{best[1]}] vs v15 {base:.2f} delta {best[0]-base:+.2f}",flush=True)
