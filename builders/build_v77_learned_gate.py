"""v77: v56 with the 2-feature hand-weighted gate replaced by the learned gate.

EVERYTHING else is byte-identical to builders/build_v56_overlap_336.py -- same seen head,
same unseen head (7-view crop-max + 336 bank + dual B2), same f=0.60, same tau=1.8,
same Sinkhorn, same full-17393 argmax. Only the ranking that decides the route changes,
so any delta is gate quality, not a moved operating point.

Gate features come from research/learned_gate_v77.py and are computed identically here
and on the train-derived holdout. The gate's bank feature is CENTER-VIEW ONLY; the unseen
head keeps its full 7-view crop-max.

  conda activate onet && python builders/build_v77_learned_gate.py
"""
import json
import os
import pickle
...
GATE_PKL = os.environ.get('GATE_PKL', 'outputs/learned_gate_v77.pkl')
TAG = os.environ.get('TAG', 'v77_learned_gate_f60_tau18')
SEEN_FRAC = 0.60
TAU = 1.8
