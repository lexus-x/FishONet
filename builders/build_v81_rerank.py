"""v81: v79 learned gate + LEARNED RE-RANKER on the unseen head.
...
and z-scores per leg, plus bank coverage. Class-disjoint CV: 33.22 -> 46.59 (+13.37).
...
from rerank_unseen_v81 import DEPLOYED_W, LEGS, pair_features  # noqa: E402
...
RERANK_PKL = os.environ.get('RERANK_PKL', 'outputs/rerank_unseen_v81_safe.pkl')
TAG = os.environ.get('TAG', 'v81_rerank_f60')
SEEN_FRAC = 0.60
TAU = 1.8
...
def main():
    ...
    print(f'reranker: K={rr["K"]} feats={len(rr["safe_idx"])} holdout gain +{rr["gain"]:.3f}', flush=True)
    ...
    Xr, rnames = pair_features(sub_legs, topk, n_photos.to(dev), sub_fused)
    assert rnames == rr['all_names'], 'reranker feature definition drifted'
    Xs = Xr[:, :, rr['safe_idx']].reshape(-1, len(rr['safe_idx'])).cpu().numpy()
