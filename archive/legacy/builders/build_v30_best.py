...
D = 'data/dl'
SEEN_FRACS = [0.72]   # calibrated optimum
...
qFS = qcat('ftshift').to(dev)   # shift-retrained encoder in the UNSEEN route (+0.52 holdout)
# taxonomy-context text anchor ("a photo of {sp}, commonly known as {common}, a fish of the family
# {fam}.") -- +0.56 holdout (outputs/unseen_promptens.log). NOTE prompt-ensembling was WORSE (dead)
# and the family-score prior was WORSE (dead, family top-1 23.8% < species 28.9%).
...
text_full = (dbnorm(qB @ TtH.t(), 0.05, 0.5) + 0.5 * dbnorm(qL @ TnL.t(), 0.05, 0.5)
             ...
             + 1.0 * dbnorm(qFS @ TtH.t(), 0.05, 0.5)
             + 1.0 * dbnorm(qB @ TTX.t(), 0.05, 0.5))
...
# rank-based route: sweep several seen-fractions. b(unseen)=14.6% is so low that ejecting a
# seen image (worth ~50-60%) to catch an unseen (worth 0.146) only pays if the ejected slice is
# >22.6% seen. The bottom pool is ~21% seen -> the 56.35% population prior over-ejects. Sweep up.
seen_set = set(seen)
for SEEN_FRAC in SEEN_FRACS:
    ...
    tag = f'v30_best{int(SEEN_FRAC*100)}'
    ...
    z.close()
    print(f'f={SEEN_FRAC}: routed_seen={int(route_seen.sum())} ({100*route_seen.float().mean():.1f}%) | '
          f'test-eval {100*(1-o_t/len(tf)):.1f}% seen-cols | unseen-eval {100*o_u/len(uf):.1f}% other-cols '
          f'-> submission_{tag}.zip', flush=True)
