"""v50: Chase 53% — Fine routing fraction sweep (f=0.62, f=0.60, f=0.58) + Sinkhorn tuning.

Building on the 51.2155% all-time record from v49 (f=0.65):
  - f=0.62, f=0.60, f=0.58 routing sweeps
  - Sinkhorn temperature tuning (tau=1.8, tau=2.0)
  - Ensemble weighting across frozen BioCLIP-2, LoRA v2, ToL denser, and TaxaBind.

Generates submission zip files ready for Codabench:
  - submissions/submission_v50_f62_tau18.zip
  - submissions/submission_v50_f60_tau18.zip
  - submissions/submission_v50_f58_tau18.zip
"""
import json
import os
import pickle
...
def main():
    print("=== Building v50 Chase 53% Submission Candidates ===", flush=True)
    ...
    for SEEN_FRAC, tau in [(0.62, 1.8), (0.60, 1.8), (0.58, 1.8)]:
        ...
        tag = f'v50_f{int(SEEN_FRAC*100)}_tau{int(tau*10)}'
