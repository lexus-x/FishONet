...
def extract(args):
    ...
    def load_one(fn):
        ...
        try:
            ...
            t2 = squash_tf(img) if args.squash_tta else None
            return fn, preprocess(img), t2
        except Exception:
            return fn, None, None

    # decode+preprocess is CPU-bound and was single-threaded, starving the GPU
    # (measured: 1 of 4 cores pegged, rest idle). Parallelize across cores;
    # GPU flush() stays on the main thread (owns the CUDA context).
    with ThreadPoolExecutor(max_workers=4) as ex:
        for i, (fn, t1, t2) in enumerate(ex.map(load_one, files)):
            if t1 is None:
                miss += 1
            else:
                buf.append(t1)
                if args.squash_tta: buf2.append(t2)
                bufn.append(fn)
...
def train(args):
    ...
    model.visual.set_grad_checkpointing(bool(args.grad_ckpt))
    model = model.to(DEV)
    ...
    resume_best = base_acc
    if args.resume:
        ck = torch.load(args.resume, map_location='cpu', weights_only=False)
        model.load_state_dict(ck['model'])
        Wc.data = ck['Wc'].to(DEV)
        resume_best = ck.get('val_acc') or base_acc  # safety checkpoints have val_acc=None (no eval run)
        print(f'RESUMED from {args.resume} (val {resume_best:.2f}%) -- running {args.epochs} more epoch(s)', flush=True)
    ...
    best = resume_best; step = 0; t0 = time.time()
    for ep in range(args.epochs):
        ...
        for batch_idx, (x, sy, gy) in enumerate(train_loader):
            ...
            if step % 400 == 0:
                # cheap safety net vs external kills: no val eval, just current weights,
                # atomic write so a mid-save kill can't corrupt it.
                tmp_latest = args.out + '.latest.tmp'
                torch.save({'model': model.state_dict(), 'Wc': Wc.detach().cpu(),
                            'species': D['species'], 'args': vars(args), 'val_acc': None,
                            'res': RES, 'ep': ep, 'step': step}, tmp_latest)
                os.replace(tmp_latest, args.out + '.latest')
                print(f'  [safety] saved {args.out}.latest @ step {step}', flush=True)
        ...
        if acc >= best:
            ...
            torch.save({'model': model.state_dict(), 'Wc': Wc.detach().cpu(),
                        ...
                        'res': RES}, tmp_out)
            os.replace(tmp_out, args.out)  # atomic: a kill mid-write corrupts only the .tmp, never args.out
...
def main():
    ...
    ap.add_argument('--grad_ckpt', type=int, default=1, help='0 = disable (faster, more VRAM)')
    ap.add_argument('--resume', default='', help='ckpt path to load weights from; --epochs = MORE epochs to run')
